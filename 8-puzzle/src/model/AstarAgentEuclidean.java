...
public class AstarAgentEuclidean implements SearchAgent {
	...
	public boolean performSearch() {
		...
		Pair<TilesState, Float> startStatePair = new Pair<>(startState , TilesStatesManager.getInstance().getHeuristicEuclidean(startState));
		...
		while(!frontier.isEmpty()) {
			...
			for (TilesState nextState : TilesStatesManager.getInstance().getNextStates(currentState)) {
				...
				if(!explored.contains(nextState.getValue()) && !inFrontier(frontier, nextState)) {
					float manhatinNextState = TilesStatesManager.getInstance().getHeuristicEuclidean(nextState);
				...
				// and select the minimum.
					...
					while(frontierIt.hasNext()) {
						...
						float euclideanNextState = TilesStatesManager.getInstance().getHeuristicEuclidean(nextState);
						if(TilesStatesManager.getInstance().sameStates(inFrontierPair.getKey(), nextState)
							...
							Pair<TilesState, Float> newPair = new Pair<>(nextState, euclideanNextState);
